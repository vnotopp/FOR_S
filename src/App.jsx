...
export default function App(){
  ...
  useEffect(()=>{ save('notes', notes); save('flashcards', flashcards); save('scheduler', scheduler); save('analytics', analytics); }, [notes, flashcards, scheduler, analytics]);

  useEffect(()=>{
    if(!analytics.dailyData || analytics.dailyData.length!==7){ const a={...analytics, dailyData:[0,0,0,0,0,0,0]}; setAnalytics(a); save('analytics', a); }
  },[]);
  ...
  return (
    <Layout page={page} setPage={setPage} theme={theme} toggleTheme={()=>setTheme(t=> t==='dark'?'light':'dark')}>
      <div className='max-w-6xl mx-auto'>
        <div className='mb-6'><h2 className='text-2xl font-bold text-bv-600'>FOR S — Siddhi's Study Space</h2></div>
